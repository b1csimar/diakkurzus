let working = document.getElementById("elaboration");
let selectitems = document.getElementsByName("studentCourse");
let chosenType = 0;
...
async function addStudent() {
    ...
    working.innerHTML = "<label for='studentInput'>Új diák neve: </label><input type='text' id='studentInput'> ";
    working.innerHTML += "<label for='selectedCourse'>Diák tantárgya:</label> <select id='selectedCourse'> name='selectedCourse' </select>";

    select = document.getElementById("selectedCourse");

    writeSelectMenu(select, courses);

    working.innerHTML += "<button onclick='saveStudent()'>Új diák mentése</button>";
}
...
async function saveStudent() {
    ...
    if (response != undefined) {
        alert("A diák hozzáadása megtörtént");
    }
    else {
        alert("A diák már hozzá lett adva");
    }
    working.style.display = "none";
}
...
async function saveCourse() {
    ...
    if (response != undefined) {
        alert("A kurzus hozzáadása megtörtént");
    }
    else {
        alert("A kurzus már hozzá lett adva");
    }
    working.style.display = "none";
}
...
async function modifyStudent() {
    ...
    working.style.display = "flex";
    working.innerHTML = "<label for='selectedStudent'>Diák:</label> <select id='selectedStudent'> name='selectedStudent' </select>";
    working.innerHTML += "<label for='studentInput'>diák új neve: </label><input type='text' id='studentInput'> ";
    ...
    let courses = await getAllFunction("courses");
    courses.unshift({ id: -1, name: "Nincs kiválasztva" });
    writeSelectMenu(courseSelect, courses);

    working.innerHTML += "<button onclick='saveModifiedStudent()'>diák módosítás mentése</button>";
}
async function saveModifiedStudent() {
    ...
    if (response != undefined) {
        alert("A diák módosítása megtörtént");
    }
    else {
        alert("A diákot nem sikerült módosítani");
    }
    working.style.display = "none";
}
...
async function modifyCourse() {
    ...
    working.innerHTML += "<button onclick='saveModifiedCourse()'>kurzus módosítás mentése</button>";
...
async function saveModifiedCourse() {
    ...
    if (response!= undefined) {
        alert("A kurzus módosítása megtörtént");
    }
    else {
        alert("A kurzust nem sikerült módosítani");
    }
...
async function deleteStudent() {
    ...
    working.style.display = "flex";
    working.innerHTML = "<label for='selectedStudent'>Diákok:</label> <select id='selectedStudent'> name='selectedStudent' </select>";

    select = document.getElementById("selectedStudent");

    writeSelectMenu(select, students);

    working.innerHTML += "<button onclick='saveDeletedStudent()'>diák törlése</button>";
}
...
async function saveDeletedStudent() {
    ...
    if (response == undefined) {
        alert("A diák törlése megtörtént");
    }
    else {
        alert("A diákot nem sikerült törölni");
    }
    working.style.display = "none";
}
...
async function saveDeletedCourse() {
    ...
    if (response == undefined) {
        alert("A kurzus törlése megtörtént");
    }
    else {
        alert("A kurzust nem sikerült törölni");
    }
    working.style.display = "none";
}